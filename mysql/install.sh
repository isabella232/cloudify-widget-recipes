...
function killMySqlProcess {
	ps -ef | grep -iE "mysql|catalina" | grep -ivE "install|gigaspaces|GSC|GSA|grep"
	if [ $? -eq 0 ] ; then 
		ps -ef | grep -iE "mysql|catalina" | grep -ivE "install|gigaspaces|GSC|GSA|grep" | awk '{print $2}' | xargs sudo kill -9
	fi  
}
