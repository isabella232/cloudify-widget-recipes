...
def installLinuxMysql(config,context,builder,currVendor,installScript,myCnfObject) {
	...
	def tomcatScript="${context.serviceDirectory}/tomcat_conf.sh"
		
	dbName = context.attributes.thisInstance["dbName"] 
	dbUser = context.attributes.thisInstance["dbUser"] 
	dbPassW = context.attributes.thisInstance["dbPassW"]
		
	builder.sequential {	
		echo(message:"mysql_install.groovy: Running ${tomcatScript} os is ${currVendor} ...")
		exec(executable:"${tomcatScript}", failonerror: "true") {
			arg(value:dbName)
			arg(value:dbUser)
			arg(value:dbPassW)
		}
