...
service {
	...
	lifecycle{
		...
		stopDetection {				
			//!ServiceUtils.arePortsFree([jdbcPort, tomcatPort] )
		...
		locator {				
			def mysqlPids = ServiceUtils.ProcessUtils.getPidsWithQuery("State.Name.re=mysql.*\\.exe|mysqld")
			println "mysql-service.groovy: current PIDs: ${mysqlPids}"
			return mysqlPids
			/*
			def tomcatPids = ServiceUtils.ProcessUtils.getPidsWithQuery("Args.*.eq=org.apache.catalina.startup.Bootstrap")						
			List<Long> merge = []
			merge.addAll(mysqlPids)
			merge.addAll(tomcatPids)
